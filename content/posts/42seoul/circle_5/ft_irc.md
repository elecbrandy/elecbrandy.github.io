...
- **동기**
    - 작업이 내가 요청한 흐름 안에서 처리되고, 결과는 함수가 끝날때 나옴.
    - 작업의 완료 시점 = 함수 리턴 시점
    - 즉, 내가 직접 처리하는 것임!
- **비동기**
    - 작업을 커널 등에 맡기고, 내 코드 흐름은 즉시 다음으로 넘어감.
    - 작업이 완료되면 알림, 콜백, 이벤트 등으로 통보받음
    - 즉, 내가 아닌 누군가가 처리하는 것임! 

정리하면, 동기 vs 비동기는 누가 **처리하느냐?** 의 문제이다.
내가 직접 처리하면 동기, OS나 다른 시스템이 처리하면 비동기인 것이다.  
____
- **블로킹**
    - 요청을 보낸 스레드가 결과가 나올 때까지 멈춘다.
    - `read()`, `accept()` 같은 함수 호출 시, 데이터가 준비될 때까지 스레드가 대기
    - 스레드는 아무 일도 못하고 함수 안에서 멈춰 있음!
- **논블로킹**
    - 요청을 보낸 스레드가 멈추지 않고, 바로 리턴받는다.
    - 함수는 그 즉시 반환되며 그 시점에서 결과가 준비 안 됐을 경우 오류 코드를 반환한다.
    - 즉, 스레드가 계속 움직일 수 있다!

블로킹 vs 논블로킹은 **기다리느냐?** 의 문제이다.
결과가 나올 때까지 멈추면 블로킹, 결과를 기다리지 않고 바로 다음 일을 하면 논블로킹인 것이다.  
...
- **동기 + 블로킹**
    ...
    - 스레드는 멈춰있음!
- **동기 + 논블로킹**
    - 내가 직접 작업 + 끝날 때까지 기다리지 않음
    - 스레드는 멈추지 않는다!
- **비동기 + 블로킹**
    - 내가 아닌 OS 등이 작업 + 끝날 때까지 기다림
    - 스레드는 멈춰있음!
- **비동기 + 논블로킹**
    - 내가 아닌 OS 등이 작업 + 끝날 때까지 기다리지 않음
    - 스레드는 멈추지 않는다!  
